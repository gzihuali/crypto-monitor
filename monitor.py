...
alerted_symbols = set()  # 每个周期内已警报的 symbol

def send_alert(symbol, price, chg, vol, period='1h'):
    message = f"[警报] 交易量延迟增长 >10 距 = 0\n币种: {symbol}\n最新价: {price}\n24h涨跌: {chg}\n24h量(USDT): {vol}\nK线周期: {period}"
    try:
        requests.get(f"https://api.telegram.org/bot{TELEGRAM_TOKEN}/sendMessage", params={"chat_id": TELEGRAM_CHAT_ID, "text": message, "parse_mode": "Markdown"})
...
def check_signals():
    ...
    alerted_symbols.clear()  # 每个监控周期清空，确保周期内只发一次
    ...
    for sym in symbols:
        try:
            ...
            if late10 == 0 and sym not in alerted_symbols:
                t = tickers.get(sym, {})
